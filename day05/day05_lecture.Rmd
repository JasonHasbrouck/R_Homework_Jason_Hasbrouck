...
- this data set has 20 rows, so the columns we are creating with just Study1, Study2, 10, and 8 is an example of how R will recycle the data to fill all rows
...
mysleep.long <- gather(mysleep.wide, "treatment", "time", c("pre", "post")) 
#or can use pre:post which will take all columns between pre and post
...
#split stuff column apart
#gsub can be used as a find and replace function - changing newrel into new_rel
who.long$stuff <- gsub("newrel", "new_rel", who.long$stuff)


#seperate function (dataframe, column name to seperate w/argument, vector of new column names)
...
# get "cell means"
#summarize, group by(data set, column 1, column 2), mean = ...)
